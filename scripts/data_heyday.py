...
with open(data_file, 'r') as f:
    ...
    for line_num, row in tqdm(enumerate(csv_reader)):
        if line_num == 0 or not row: continue
        docs.append(row[0])

# Tokenize
init_docs = [re.findall(r'''[\w']+|[.,!?;-~{}`´_<=>:/@*()&'$%#"]''', docs[doc]) for doc in
             range(len(docs))]
...
num_docs = len(docs)
trSize = int(np.floor(num_docs * .85))
tsSize = int(np.floor(num_docs * .10))
...
docs_ts = [[word2id[w] for w in init_docs[idx_permute[idx_d + trSize]].split() if w in word2id] for idx_d in
           range(tsSize)]
docs_va = [[word2id[w] for w in init_docs[idx_permute[idx_d + trSize + tsSize]].split() if w in word2id] for idx_d in
           range(vaSize)]
